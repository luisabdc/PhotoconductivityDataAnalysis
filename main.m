...
% please read the accompagnying document.)
...
global muTauVsRMSPlot muTauVsRlightPlot
    %If using the simplified version, use Rlight and not
    %Rlightfilter (Rlight would be equivalent to full illumination)
...
% SIMPLIFIED VERSION: uncomment the following line, and comment out the
% loop later on in the code.
%T = 310; %K
...
% The following code to find the best values for tau and mu iterate through
% smaller and smaller ranges, and this number controls how many iterations
% actually take place. It is possible that more than one iteration is not
% necessary.
...
% For the simplified version, uncomment this (specifies the fully illuminated resistance):
%specificT = find(temperature == T/100);
%specificIll = find(illumination == 8);
%Rlight = sample.RList(specificT,specificIll);%ohms
%Rdark = sample.RList(specificT,1); %ohms
...
for iT=1:Tn
    ...
    for iI = 1:In
        ...
        % In the case that numberOfIterations = 1, the following section is
        % only repeated once. If not, the later runs will employ a narrower
        % range for the modeling.
        ...
        % Now all the figures are saved for each combination of temperature
        % and illumination studied in a general folder for the experiment.
        mkdir(['FiguresFor' name]);
        nameOfDir = sprintf('figures_for_T_%d_and_ill_%.0f\n',T,illPercent);
        % Note the illumination percent that shows up here is the one taken
        % as "light" in the comparison.
        savefig(contourPlot,['FiguresFor' name '/' sprintf('contour_plot_for_T_%d_and_ill_%.2f.fig',T,illPercent)]);
        savefig(muTauVsRMSPlot,['FiguresFor' name '/' sprintf('muTau_RMS_plot_for_T_%d_and_ill_%.2f.fig',T,illPercent)]);
        savefig(muTauVsRlightPlot,['FiguresFor' name '/' sprintf('muTau_Rlight_plot_for_T_%d_and_ill_%.2f.fig',T,illPercent)]);
    end
end
table
nameOfFile = sprintf('geometric_constants_for_%s.mat\n',name);
