...
% =========================================================================   
    ...
    % for dark , T starts at 3rd underscore, ill starts at 4th
    % underscore. for filters and normal, T starts at 4th and ill starts at
    % 5th
        underscores = strfind(fileList(i).name, '_');
        % only searches for K after the first underscore, to avoid
        % confusion with filename
        K_location = strfind(fileList(i).name(underscores(3)+1:end), 'K');         
        temp_index_begin = underscores(3)+1; % the temperature begins right after the first _
        temp_index_end = underscores(3)+K_location-1; % the temperature ends right before K

        % the illumination index begins right after the 'ill', only searches
        % after third underscore, to avoid confusion with file name
        ill_location = strfind(fileList(i).name(underscores(4)+1:end), 'ill'); 
        ill_index_begin = underscores(4)+ ill_location+3; % illumination index begins at O - only second index counts      
        ill_index_end = underscores(5) - 1; % illumination index ends right before the final underscore
        %ill_index of 0 is actually 65535
        
        if isempty(ill_location)
            warning(['ill_location is empty. Check underscore numbers on line 69'])
        end
...
%%=========================================================================
        % same T over every illumination value
%%=========================================================================
illi = 0;
 for illum = ill1(1:end)
        ...
        subplotNum = 1;          
         ...
         voltage = VI1{Ti,illum,1};
         current = VI1{Ti,illum,2}; 
         
         subplot(1,2,subplotNum);% Consider: if length(ill1) is even, make rows
            
         hold on
            
         h = plot(voltage,current);
         set(h,'Color',[illi/length(ill1) 0 1-(illi/length(ill1))]);
       
         title([num2str(T(Ti))*100,'K for every illumination value']);
            end                                 
%        end % of all plots        
  

%%=========================================================================
        % same illumination value over every T
%%=========================================================================
...
for temp = T1(1:end)
    tempi = tempi+1;
        ...
        illum = 1;
         ...
         voltage = VI1{tempi,illum,1};
         current = VI1{tempi,illum,2}; 
         ...
         h = plot(voltage,current);
         set(h,'Color',[tempi/length(T1) 0 1-(tempi/length(T1))]);
                  
         title(['Same ill for T of ', num2str(Ti*100),'K']);
            end 
